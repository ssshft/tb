...
BinanceUFTradingClient::BinanceUFTradingClient(){
...
bool BinanceUFTradingClient::Initialize(AccountCfg& cfg, sm::SecurityManager *smc){
    this->smc = smc;
    //static values
    m_wssb_url = "/ws/";
//    m_ord_url = "/api/v3/order";
    m_ord_url = "/fapi/v1/order";
    //m_cxt_url = "/api/v3/order";
//    m_trans_url = "/sapi/v1/asset/transfer";
//    m_wsLiskey_url = "/api/v3/userDataStream";
    m_wsLiskey_url = "/fapi/v1/listenKey";
    m_queryord_url[0] = "/fapi/v1/order";
    m_queryord_url[1] = "/fapi/v1/openOrders";
    m_queryord_url[2] = "/fapi/v1/allOrders";
//
    m_balanceUrl = "/fapi/v2/account";
    m_positionUrl = "/fapi/v2/positionRisk";
    m_adlquantileUrl = "/fapi/v1/adlQuantile";
...
void BinanceUFTradingClient::Run() {
    std::thread monitorThread(&BinanceUFTradingClient::monitor, this);
    monitorThread.detach();
}



void BinanceSpotTradeUnit::gen_listen_key() {
    try{
        // http_client restclient(m_curcfg.restBaseUrl);
        uri_builder builder(m_wsLiskey_url);
        http_request request(methods::POST);
        request.set_request_uri(builder.to_string());
        request.headers().add("X-MBX-APIKEY", m_curcfg.apiKey);
        // req.set_method("application/x-www-form-urlencoded");
        ...
        if(response.status_code() == status_codes::OK){
            ...
            if (v.has_field("listenKey")){
                m_listenkey = v.at("listenKey").as_string();
                LOG_DEBUG("accountId:%s,listenkey:%s", m_curcfg.accountId, m_listenkey.c_str());
            }
            else{
                m_IsConnected = false;
            }
        ...
        m_IsConnected = false;
        LOG_ERROR("Error exception:%s", e.what());
    }
}

void BinanceSpotTradeUnit::keep_listen_key() {
    try{
        if (m_curcfg.instTypeEnum == InstType_SPOT){
            gen_listen_key();
            return;
        }
        // http_client restclient(m_curcfg.restBaseUrl);
        ...
        request.headers().add("X-MBX-APIKEY", m_curcfg.apiKey);
        json::value obj;
        obj["listenKey"] = json::value::string(m_listenkey);
        request.set_body(obj.serialize());
        LOG_DEBUG("%s", obj.serialize().c_str());
        // request.set_method("application/x-www-form-urlencoded");
        http_response response = hotHttpClient->request(request);
        auto code = response.status_code();
        if(code == status_codes::OK || code == status_codes::BadRequest){
            // auto previousTask = response.extract_json();
            json::value const &v = response.extract_json().get();
            auto errcode = 0;
            string errmsg = "";
            if (v.has_field("code")){
            ...
            LOG_DEBUG("accountId:'%s' listenkey:'%s' keep_listen_key response: errCode='%d',errMsg='%s'"
            , m_curcfg.accountId,m_listenkey.c_str(), errcode, errmsg.c_str());
            if(errcode != 0){
                m_IsConnected = false;
                gen_listen_key();
            }
...
void BinanceUFTradingClient::sub_websocket(){
    try{
        ...
        .then([&]() {
            ...
            f = std::bind(&BinanceUFTradingClient::on_websocket_msg, this, placeholders::_1);
            wsClient.set_message_handler(f);
            std::function<void (websocket_close_status close_status,
                                const utility::string_t& reason,
                                const std::error_code& error)> c;
            c =  std::bind(&BinanceUFTradingClient::on_close_msg,this
...
void BinanceUFTradingClient::monitor() {
    while(1){
        try{
...
void BinanceUFTradingClient::on_websocket_msg(const websocket_incoming_message& msg){
    try{
...
bool BinanceUFTradingClient::get_balances(pubsub::TCommand &tcmd)//vector<Balance> &balanceVec
try {
    // http_client restclient(m_curcfg.restBaseUrl);
...
bool BinanceUFTradingClient::get_positions(pubsub::TCommand &tcmd)
try {
    // http_client restclient(m_curcfg.restBaseUrl);
    http_request request(methods::GET);
    // request.headers().add("Connection", "Keep-Alive");
...
bool BinanceUFTradingClient::get_adlquantile(pubsub::TCommand &tcmd)
try {
    // http_client restclient(m_curcfg.restBaseUrl);
    http_request request(methods::GET);
    // request.headers().add("Connection", "Keep-Alive");
...
void BinanceUFTradingClient::add_new_order(pubsub::TCommand &tcmd){
...
void BinanceUFTradingClient::cancel_order(pubsub::TCommand &tcmd){
            ...
            if(tcmd.body.cancelOrder.cancelOrderTypeEnum == pubsub::COT_ONE_INST){
                ...
                if (code == status_codes::OK || code == status_codes::BadRequest
                    ...
                    if (rawData.HasMember("code")){
                        ...
                        rcmd.body.cancelOrder.tradePrice   = stod(rawData["avgPrice"].GetString())*info.reduceNumber;
...
void BinanceUFTradingClient::query_one_order(pubsub::TCommand &tcmd){
        ...
        if(smc->get_instrument_info(tcmd.header.exchangeTypeEnum, tcmd.header.instTypeEnum, tcmd.body.queryOrder.instId, info)){
            ...
            || code == status_codes::TooManyRequests || code == status_codes::Unauthorized){
                ...
                if(v.has_field("code")) {
                    ...
                    rcmd.body.queryOrder.tradePrice   = stod(v.at("avgPrice").as_string().c_str())*info.reduceNumber;
...
void BinanceUFTradingClient::query_multi_orders(pubsub::TCommand &tcmd){

}

void BinanceUFTradingClient::query_order(pubsub::TCommand &tcmd){
