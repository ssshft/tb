#include "GateioCSwapTradingClient.h"

GateioCSwapTradingClient::GateioCSwapTradingClient(){
...
bool GateioCSwapTradingClient::Initialize(AccountCfg& cfg, sm::SecurityManager *smc){
    this->smc = smc;
    m_positionsUrl = "/api/v4/futures/btc/positions";
    m_accountsUrl = "/api/v4/futures/btc/accounts";
    m_orderUrl = "/api/v4/futures/btc/orders";
//    m_cancelOrderUrl = "/api/v4/futures/usdt/orders/";
    m_curcfg = cfg;
    return true;
}

void GateioCSwapTradingClient::Run() {
    std::thread monitorThread(&GateioCSwapTradingClient::monitor, this);
...
void GateioCSwapTradingClient::sub_websocket()//{// websocket_client websocket_callback_client &wsclient)
try
{
    ...
    .then([&]() {
        ...
        f = std::bind(&GateioCSwapTradingClient::on_websocket_msg, this, placeholders::_1);
        ...
        c =  std::bind(&GateioCSwapTradingClient::on_close_msg,this
    ...
    m_IsConnected = true;
    LOG_INFO("connected with gateio btc swap api");
...
websocket_outgoing_message GateioCSwapTradingClient::sub_orders_channel() {
...
websocket_outgoing_message GateioCSwapTradingClient::sub_positions_channel() {
...
websocket_outgoing_message GateioCSwapTradingClient::sub_position_close_channel() {
...
websocket_outgoing_message GateioCSwapTradingClient::sub_balance_channel() {
...
websocket_outgoing_message GateioCSwapTradingClient::sub_trades_channel() {
...
void GateioCSwapTradingClient::monitor() {
    while(1){
        try{
            LOG_INFO("start to connect with gateio btc swap api");
            sub_websocket();

            while(m_IsConnected){
                sleep(10);
                if(!m_IsConnected){
                    LOG_ERROR("gateio btc swap ws disconnected, will reconnect it now");
                    break;
                }
                else{
...
void GateioCSwapTradingClient::on_websocket_msg(const websocket_incoming_message& msg)
try{
...
//                                                                / rcmd.body.orderResponse.volumeTraded;
                ...
                //{"time":1649401766,"channel":"futures.usertrades","event":"update","result":[{"id":"3587468",
                // "order_id":"590615217","contract":"BTC_USDT","create_time":1649401766,"create_time_ms":1649401766833,
                // "size":-1,"role":"taker","price":"43668.5","text":"web"}]}
                const rapidjson::Value &data = rawData["result"];
                for(rapidjson::SizeType i = 0; i < data.Size(); i++){
                    string originInstId = data[i]["contract"].GetString();
                    md::InstrumentInfo info;
...
//                        MsgExt msgExt;
                        ...
                        cmd.header.instTypeEnum = InstType_BTC_SWAP;
                        ...
                        if(data[i].HasMember("text")){//实盘可能没有这个字段
                        ...
                        //TODO
//                        cmd.body.trade.offsetFlag = OffsetFlag_OPEN;//crypto::str_cmp(data[i][""].GetString())
//                        cmd.body.trade.direction = crypto::str_cmp(data[i]["side"].GetString(),"sell") ? Direction_SHORT : Direction_LONG;
                ...
                for(rapidjson::SizeType i = 0; i < data.Size(); i++){
                    ...
                    else if(crypto::has_str(text.c_str(),"BTC")){
                        strcpy(cmd.body.balance.currency, "BTC");
                    }
                    else{
                        LOG_ERROR("%s",s.c_str());
                        continue;
                    ...
                    g_rptInnerQueue.push(cmd);
//                    if(cmd.body.balance.available > ZERO_NUM){
//                        g_rptInnerQueue.push(cmd);
//                    }
                }
            }
...
void GateioCSwapTradingClient::on_close_msg(websocket_close_status close_status,
                  const utility::string_t& reason, const std::error_code& error)
try{
    m_IsConnected = false;
    LOG_ERROR("C swap recv CloseMsg, reason:%s ",reason.c_str() );
}
catch (exception &e){
    LOG_ERROR("%s", e.what());
}
...
#if 0
string GateioCSwapTradingClient::get_signature_ws(const char *channel, const char *event, const char * time){
    string s("");
    s.append("channel=").append(channel).append("&event=")
            .append(event).append("&time=").append(time);
//    cout << s << endl;
//    std::string hmacsha512hex = hmac<sha512>::calc_hex(s, this->_apiSecret);
...
string GateioCSwapTradingClient::get_signature_rest(const char *method, const char *url, const char * time,
...
void GateioCSwapTradingClient::ping(){
    if(m_IsConnected){
        websocket_outgoing_message outMsg;
...
void GateioCSwapTradingClient::pong(){
    if(m_IsConnected){
        websocket_outgoing_message outMsg;
...
bool GateioCSwapTradingClient::get_positions()//vector<Balance> &balanceVec
try {
...
bool GateioCSwapTradingClient::get_accounts()
try {
    http_client restclient(m_curcfg.restBaseUrl);//bUrl
    http_request request(methods::GET);
    request.headers().add("Accept","application/json");
...
//                    g_rptInnerQueue.push(cmd);
//                    if(cmd.body.balance.available > ZERO_NUM || cmd.body.balance.frozen > ZERO_NUM){
//                        g_rptInnerQueue.push(cmd);
//                    }
                }
                else{
...
void GateioCSwapTradingClient::add_new_order(pubsub::TCommand &cmd){
    pubsub::RCommand rcmd;
    memset(&rcmd,0,sizeof(pubsub::RCommand));
...
//            std::unordered_map<std::string, std::string> params;
            ...
            value["price"] = json::value::string(crypto::getFixedPrecision(cmd.body.newOrder.limitPrice, info.tickSize));
//            rcmd.body.newOrder.volumeTotal = cmd.body.newOrder.volumeTotal;
//            rcmd.body.newOrder.limitPrice = cmd.body.newOrder.limitPrice;
            ...
            if(cmd.body.newOrder.offsetFlag == OffsetFlag_OPEN){
                ...
                if(cmd.body.newOrder.direction == Direction_LONG){
                    value["size"] = json::value::string( crypto::getFixedPrecision(cmd.body.newOrder.volumeTotal, info.lotSize)) ;
                }//
            ...
            request.set_request_uri(builder.to_string());
            LOG_DEBUG("url:%s%s,params:%s",m_curcfg.restBaseUrl.c_str(), m_orderUrl.to_string().c_str(), value.serialize().c_str());
            restclient.request(request)
            ...
            .then([&](pplx::task<json::value> previousTask) {
                ...
                if(v.has_field("id")){
                    ...
                    rcmd.body.newOrder.volumeTotal = size;
...
void GateioCSwapTradingClient::cancel_order(pubsub::TCommand &cmd){
    pubsub::RCommand rcmd;
    memset(&rcmd, 0, sizeof(pubsub::RCommand));
    rcmd.header.exchangeTypeEnum = cmd.header.exchangeTypeEnum;
    rcmd.header.instTypeEnum = cmd.header.instTypeEnum;
...
void GateioCSwapTradingClient::query_multi_orders(pubsub::TCommand &cmd){
    try {
        md::InstrumentInfo info;
...
void GateioCSwapTradingClient::query_one_order(pubsub::TCommand &cmd){
    pubsub::RCommand rcmd;
    memset(&rcmd,0,sizeof(pubsub::RCommand));
    strcpy(rcmd.header.accountId, cmd.header.accountId);
    rcmd.header.instTypeEnum = cmd.header.instTypeEnum;
...
//            queryStr.append(info.originInstId);
            ...
            restclient.request(request)
            .then([&](http_response response) -> pplx::task<json::value> { auto code = response.status_code();
//                        LOG_DEBUG("query_order response:'%s' ",response.to_string().c_str());
...
//                return pplx::task_from_result(json::value());
            ...
            .then([&](pplx::task<json::value> previousTask) {
                ...
                if(v.has_field("id") && v.has_field("status")){
                    ...
                    string orderId = std::to_string(v.at("id").as_number().to_int64());
                    strcpy(rcmd.body.orderResponse.orderId, orderId.c_str());
                    strcpy(rcmd.body.orderResponse.clientOrderId, v.at("text").serialize().c_str());
                    ...
                    double left = stod(v.at("left").serialize().c_str());//v.at("left").as_integer();
                    left = left > 0 ? left : -left;
                    rcmd.body.orderResponse.volumeTraded = rcmd.body.orderResponse.volumeTotal - left;
                    rcmd.body.orderResponse.tradePrice   = stod(v.at("fill_price").as_string().c_str() );
                    rcmd.body.orderResponse.reduceOnly = v.at("is_reduce_only").as_bool();
                    ...
                    rcmd.body.orderResponse.insertTime = stol(v.at("create_time").serialize().c_str()) * 1000*1000;
                    if(v.has_field("finish_time")){
                        rcmd.body.orderResponse.updateTime = stol(v.at("finish_time").serialize().c_str()) * 1000*1000; //v.at("finish_time").as_double() * 1000*1000;
                    }
                    else{
...
void GateioCSwapTradingClient::query_order(pubsub::TCommand &cmd){
    pubsub::RCommand rcmd;
    memset(&rcmd, 0, sizeof(pubsub::RCommand));
    strcpy(rcmd.header.accountId, cmd.header.accountId);
    rcmd.header.instTypeEnum = cmd.header.instTypeEnum;
